...
for(mt in mean_type){
  for(kt in kernels_v){
    fn <- paste0("ALV2_26_9_prior_gam_np_h_low_rank_", mt, "_", kt, ".RData")
...
p1 <- dat_comp_nn%>%
  ...
  labs(y = "Relative Error (%)",
       x = "Model",
       title = "Accuracy on Binary Composition Alloys")+
  theme(legend.position = "bottom",
        axis.text.x = element_text(size = 12),
        legend.text = element_text(size=10),
        legend.title = element_text(size =10))
...
dat_pm2%>%
  filter(property %in% c("epsilon[u]", "U[T]"),
         Step == 0)%>%
  ggplot(aes(x= Nb,
             y = val))+
  geom_line(aes(color = Step),
  ...
  geom_point(size = 1.5,
             ...
             color = "blue")+
  labs(y = "Value",
       title = "e) Properties Prediction of MoNb Alloys")+
  facet_wrap(~property,labeller = label_parsed,
             scales = "free", nrow = 1)+
...
dat_pm2%>%
  ...
  geom_point(size = 1.5,
             ...
             color = "red")+
  labs(y = "Value",
       title = "e) Properties Prediction of VW Alloys")+
  facet_wrap(~property,labeller = label_parsed,
             scales = "free", nrow = 1)+
  theme(legend.position = "none")
```
...
qz <- qnorm(0.05/2)
props_in <- c("ln(epsilon[u])", "beta[1]")
pc <- pred_ms_c%>%
  filter(Step == 0,
