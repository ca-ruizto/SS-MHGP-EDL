...
for(p in n_pol){
  ...
  dat_ls[[k]] <- tibble(length_scale = exp(pol[[p]]$theta ),
                        dimension = 1:length(pol[[p]]$theta),
                        Model = p)
  
  for(i in np_vals){
    message(paste("N-points", i, "at", Sys.time()))
    val <- c(val, IMSPE_MV_Sampling(pol[[p]],
                             n_points = i))
    gc()
  }
  imspe_samp_dat[[k]] <- tibble(imspe = val ,
                                Model = p,
                                n_samples = np_vals)
}

imspe_samp_dat <- bind_rows(imspe_samp_dat)
...
gridExtra::grid.arrange(plot_int, plot_bad, nrow = 1)
```
...
IMSPE_MV_NI(pol[[4]], max_eval = 3e4, return_object = F)
