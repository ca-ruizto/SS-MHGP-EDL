...
```{r}
nn_bin_t <- read.csv(here::here("Results",
                                "NN_Experts",
                                "dat_test_bin_even_t_predictions.csv"))

nn_bin_c <- read.csv(here::here("Results",
                                "NN_Experts",
                                "dat_test_bin_even_c_predictions.csv"))

nn_qq_t <- read.csv(here::here("Results",
                                "NN_Experts",
                                "dat_test_qq_t_predictions.csv"))

nn_qq_c <- read.csv(here::here("Results",
                                "NN_Experts",
...
gp_mu_nn_t <- bind_rows(nn_bin_t[,c(1,mu_ids)],
                        nn_qq_t[,c(1,mu_ids)])%>%
              arrange(alloy_cnt)
gp_mu_nn_t <- as.matrix(gp_mu_nn_t[,-1])

gp_var_nn_t<- bind_rows(nn_bin_t[,c(1,var_ids)],
                        nn_qq_t[,c(1,var_ids)])%>%
              arrange(alloy_cnt)
gp_var_nn_t <- as.matrix(gp_var_nn_t[,-1])

gp_mu_nn_c <- bind_rows(nn_bin_c[,c(1,mu_ids)],
                        nn_qq_c[,c(1,mu_ids)])%>%
              arrange(alloy_cnt)
gp_mu_nn_c <- as.matrix(gp_mu_nn_c[,-1])

gp_var_nn_c<- bind_rows(nn_bin_c[,c(1,var_ids)],
                        nn_qq_c[,c(1,var_ids)])%>%
              arrange(alloy_cnt)
gp_var_nn_c <- as.matrix(gp_var_nn_c[,-1])
```


```{r}

res_nn_t <- predict_mod_het_gp_sampling_fun(Xdesign = dat_pred[,c(metals,"alloy_cnt")],
                                            gp_mu = gp_mu_nn_t,
                                           gp_var = gp_var_nn_t,
                                           step_id = "NN",
                                           test_type = "Tension",
...
res_nn_c <- predict_mod_het_gp_sampling_fun(Xdesign = dat_pred[,c(metals,"alloy_cnt")],
                                            gp_mu = gp_mu_nn_c,
                                           gp_var = gp_var_nn_c,
                                           step_id = "NN",
